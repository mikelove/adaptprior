...
```{r scatter}
ggplot(dat, aes(x=true, y=log2FC, col=category)) + geom_point(size=2) +
...
```{r yeast-ma2, echo=FALSE}
ggplot(res.df, aes(log10mean, log2FoldChange, col=FDR.05, ymin=ymin, ymax=ymax)) +
  geom_point(alpha=.3) + geom_errorbar(alpha=.3) + 
  geom_hline(yintercept=0, col="red") + xlim(2,6) + ylim(-2,2) +
  scale_color_manual(values=c("black", "red"))
```

## Problems with the Normal prior

## ashr prior

## apeglm prior

## Issues with estimating scale

## Performance on the yeast data
