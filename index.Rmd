...
```{r scatter}
ggplot(dat, aes(x=true, y=log2FoldChange, col=category)) + geom_point(alpha=.3,size=2) +
...
* ShrinkBayes [@vandewiel2012]
    - Dirac–Gaussian, Gaussian–Dirac–Gaussian
	- Gamma–Dirac-reverse Gamma, Dirac-central Laplace
* ShrinkBayes [@vandewiel2014]
    - automated estimation of parametric priors
    - non-equal mixture proportions for + and - effects
	- Spike-Gauss-Gauss, and Spike-Nonparametric
* Models fit using Integrated Nested Laplace Approximation `INLA`
...
* with ZINB single cell simulation, bimodal LFC dist'n
